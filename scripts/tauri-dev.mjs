...
const startPort = parsePort(process.env.AMEYA_DEV_PORT);
...
const env = {
  ...process.env,
  AMEYA_DEV_PORT: String(port),
  AMEYA_DEV_STRICT_PORT: "1",
};

if (process.env.AMEYA_DEV_DRY_RUN === "1") {
  ...
  console.log(
    JSON.stringify({
      ...
      devUrl,
      env: {
        AMEYA_DEV_PORT: env.AMEYA_DEV_PORT,
        AMEYA_DEV_STRICT_PORT: env.AMEYA_DEV_STRICT_PORT,
      },
    }),
  );
  process.exit(0);
...
console.log(`[ameya] Starting Tauri dev server at ${devUrl}`);
...
child.on("error", (error) => {
  console.error(`[ameya] Failed to start Tauri dev: ${error.message}`);
  process.exitCode = 1;
});
